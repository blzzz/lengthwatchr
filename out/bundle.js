...
/***/ function(module, exports, __webpack_require__) {
	...
	var $, Article, ArticleCollection, Backbone, ItemContainerView, ItemView, departments, jQuery, links;
	...
	Article = Backbone.Model.extend({
	  initialize: function() {
	    var i, images, img, img_urls, j, json, leadImage, len, len1, mediaReferences, name, ref, results, textVolume;
	    json = this.toJSON();
	    images = [];
	    leadImage = json.leadImage;
	    mediaReferences = json['media-references'];
	    if (leadImage) {
	      images.push(leadImage);
	    }
	    if (mediaReferences) {
	      images = images.concat(_.values(mediaReferences.image));
	    }
	    img_urls = [];
	    if (images.length > 0) {
	    ...
	    this.set('images', img_urls);
	    this.set('mainDepartment', json.departmentNames[0]);
	    textVolume = $(json.body).text().length;
	    console.log(textVolume);
	    this.set('text_volume', textVolume);
	    this.set('sentences', $(json.body).text().split('.').length);
	    $(json.body).find('a[href]').each(function() {
	      return links.push($(this).attr('href'));
	    });
	    if (json.departmentNames) {
	      ref = json.departmentNames;
	      results = [];
	      for (j = 0, len1 = ref.length; j < len1; j++) {
	        name = ref[j];
	        if (!departments[name]) {
	          results.push(departments[name] = 1);
	        } else {
	          results.push(departments[name]++);
	        }
	      }
	      return results;
	    }
	...
	ItemContainerView = Backbone.View.extend({
	  ...
	  render: function() {
	    var groups;
	    groups = this.collection.groupBy('mainDepartment');
	    delete groups['News-Ticker'];
	    console.log(groups);
	    _.each(groups, function(models, department) {
	      ...
	      return _.each(models, function(model) {
	        ...
	        return this.subviews.push(itemView);
	      }, this);
	    }, this);
	    console.log(links);
	...
	ItemView = Backbone.View.extend({
	  ...
	  render: function() {
	    ...
	    if (json.isBreakingNews) {
	      this.$el.addClass('breaking');
	    }
	    if (json.departmentNames && json.departmentNames[0] === 'News-Ticker') {
